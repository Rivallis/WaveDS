...
def train_on_test(base_model: torch.nn.Module,
    ...
    batch_wavefields = 16
    train_loader = iter(torch.utils.data.DataLoader(dataset_train, batch_size=batch_wavefields, shuffle=False, num_workers=args.num_workers))
    val_loader = iter(torch.utils.data.DataLoader(dataset_val, batch_size=batch_wavefields, shuffle=False, num_workers=args.num_workers))
    ...
    for data_iter_step in range(iter_start, dataset_len):
        ...
        # test_samples = test_samples.unsqueeze(0)
        ...
        for step_per_example in range(args.steps_per_example * accum_iter):
            ...
            if (step_per_example + 1) % accum_iter == 0:
                with torch.no_grad():
                    ...
                    acc1 = (test_label.cpu().detach()[0] == all_pred[0]).sum().item()
